...
- `cmd_list_show.go` — original combined file for `cmdList`, `runList`, `runListUser`, `cmdShow`, `runShow`, and the `sortedKeys` generic helper. It was later split into [cmd_list.go](../../cmd_list.go), [cmd_show.go](../../cmd_show.go), and [sortutil.go](../../sortutil.go).
...
Test files: [format_test.go](../../format_test.go) and the original `cmd_list_show_test.go` — 20+ cases covering formatting helpers, list output, filter behavior, show column content, and refusal on bad check state. The combined test file was later split into [cmd_list_test.go](../../cmd_list_test.go) and [cmd_show_test.go](../../cmd_show_test.go).
...
- **CLI polish**:
  ...
  - [cmd_check.go](../../cmd_check.go), [cmd_init_ipsets.go](../../cmd_init_ipsets.go), [cmd_list.go](../../cmd_list.go), [cmd_show.go](../../cmd_show.go), and [cmd_create.go](../../cmd_create.go): unsupported `--dry-run` is rejected with exit code 2.
  - [cmd_init_ipsets.go](../../cmd_init_ipsets.go), [cmd_list.go](../../cmd_list.go), [cmd_show.go](../../cmd_show.go): successful setup/read-only commands now print concise final result lines.
  - [cmd_list.go](../../cmd_list.go): missing filtered users now report on stderr with `list: FAILED`.
- **Docs**:
  - [README.md](../../README.md): wording now reflects the implemented tool and documents `--dry-run` support/rejection.
  - [docs/NOTES.md](../NOTES.md): extracted durable implementation conventions from `PROGRESS.md` for future work without the phase history.
- **Smoke-style tests**:
...
Phase 11 is real-host smoke checking and final build/install verification on a controlled Linux host. Use [docs/NOTES.md](../NOTES.md) for durable implementation conventions before making further changes.
...
| `sortedKeys` | generic helper in [sortutil.go](../../sortutil.go); requires Go 1.18+ |
