...
- `list -r <target>` is a list-local flag that prints users with effective
  ...
  users with `(*)` when access to a VM/resource target comes through `*`.
  It also annotates users when access to the requested target is inherited from
  user groups with no direct user grant for that same target.
